...
export const getById = async (id: Todo['id']) =>
  context.dapr.invoker.invoke(SERVICE_APP_ID, METHODS.TodoGetById, HttpMethod.POST, { id }) as Promise<{
    payload: Todo;
  }>;
...
export const deleteById = async (id: Todo['id']) =>
  context.dapr.invoker.invoke(SERVICE_APP_ID, METHODS.TodoDeleteById, HttpMethod.POST, { id }) as Promise<{
    payload: Todo;
  }>;

export const updateById = async (id: Todo['id'], data: Partial<Todo>) =>
  context.dapr.invoker.invoke(SERVICE_APP_ID, METHODS.TodoUpdateById, HttpMethod.POST, { id, data }) as Promise<{
    payload: Todo;
  }>;
